...
\section{Type-directed adaptation protocol for REST APIs} % (fold)
\label{sec:type-directed_adaptation_protocol_for_REST_APIs}

\begin{figure}[htbp]
    \centering
    \includegraphics[height=5in]{simple_sig}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=3in]{complez_sig}
...
\subsection{Approaches under assessment} % (fold)
...
\subsection{Benchmark platform architecture} % (fold)
...
\subsection{Experiment Specification} % (fold)
...
\subsection{Test Implementations} % (fold)
...
The benchmark platform should be able to perform experiments in a local kubernetes cluster or across all remote cluster providers without requiring any changes to the experiment artifacts.
